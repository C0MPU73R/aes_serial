...
int main(int argc, char *argv[]) {
	...
	unsigned int shellcode_length = readfile("bin/payload.bin", shellcode, 0);
  ...
  if (!handle) {
    ...
    printf("."); fflush(stdout);
		ret = libusb_control_transfer(handle, 0xA1, 2, 0, 0, data, 0x800, 1000);
		if (ret < 0) break;
		fwrite(data, 1, 0x100, fOut);
		addr += 0x800;
    break;
	} while (addr < 0x10000);
  fclose(fOut);
